...
<body style="font-family: Arial, sans-serif; background-color: #f5f5f5; padding: 40px;">
  <div style="max-width: 500px; margin: auto; background: white; padding: 30px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
     ...
     * @returns 
     */
    otpCode: ({ email, code }: { email: string, code: string }): { text: string, html: string } => {
        // ---
        const text = code;
        // ---
  ...
  <div style="max-width: 500px; margin: auto; background: white; padding: 30px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1); text-align: center;">
     ...
     * @returns 
     */
    newSignIn: ({ email, user_agent, ip_address }: { email: string, user_agent: string, ip_address: string }): { text: string, html: string } => {
...
We noticed that a new device attempted to sign-in to your account. Below are the details:
 ...
 - Device: ${os}
 - IP: ${ip_address}
 - Time: ${DateUtils.format("%d %M %Y at %H:%i")}
...
<ul style="padding-left: 20px; color: #444;">
  ...
  <li><strong>Time:</strong> ${DateUtils.format("%d %M %Y at %H:%i")}</li>
</ul>
...
<p style="color: #999;">The Vortex Vault team</p>`;
     ...
     * @returns 
     */
    newPasskeyAdded: (email: string): { text: string, html: string } => {
        const text =
            `Hello ${email.split('@')[0]},
...
<p style="color: #999;">The Vortex Vault team</p>`;
     ...
     * @returns
     */
    otpFailedAttempt: ({ email, ip_address }: { email: string, ip_address: string }): { text: string, html: string } => {
        const text =
            `Hello ${email.split("@")[0]},
...
<p style="color: #999;">The Vortex Vault team</p>`;
     ...
     * @returns
     */
    changePassword: ({ email }: { email: string }): { text: string, html: string } => {
        const text =
            `Hello ${email.split("@")[0]},
...
<p style="color: #999;">The Vortex Vault team</p>`;
     ...
     * @returns
     */
    deleteAccount: ({ email }: { email: string }): { text: string, html: string }=> {
        const text =
            `Hello ${email.split("@")[0]},
